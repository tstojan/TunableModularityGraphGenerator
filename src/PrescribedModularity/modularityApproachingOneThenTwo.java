...
public class modularityApproachingOneThenTwo {
	...
	public modularityApproachingOneThenTwo(int _L, int _c, double _M, double _t) throws IOException
	{
		L = _L;
		c = _c;
		M = _M;
		...
		System.out.println("------------------------ StartReplacing --------------------------");
		System.out.println();
    ...
    private double initialize() {
		...
		else if(r<=(c/2))
		{
			while(i<=(c-r))
			{
				...
				if(i<=r)
				{
					C[c-i+2][i] = C[i][c-i+2] = 1;
					internalLinks[c-i+2] = k;
				}
				
				C[c-r][c-r+1] = C[c-r+1][c-r] = 1;
				internalLinks[c-r+1] = k-1;
				i++;
			}
	...
	private boolean approxAlgorithm(double max_m) throws IOException {
		...
		while(Math.abs(curr_m-M)>toler)
		{
			...
			if(curr_m>M)
			{
				...
				//System.out.println("m = " + curr_m + " decr");
			}
			else
			{
				...
				//System.out.println("m = "+curr_m + " incr");
			}
